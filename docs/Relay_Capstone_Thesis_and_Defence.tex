...
\begin{titlepage}
  ...
  \vfill
  \begin{tabular}{ll}
    \textbf{Dhruv Mann} & Project manager; backend testing and CI/CD \\
    \textbf{Arshia Bar} & Retrieval-augmented generation and AI testing \\
    \textbf{Dipak Prasad} & Data analysis, security, and infrastructure \\
    \textbf{Smeet Patel} & Frontend, UI, and accessibility testing \\
  \end{tabular}
  \vfill
...
\begin{longtable}[]{@{}p{0.16\textwidth}p{0.23\textwidth}p{0.25\textwidth}p{0.28\textwidth}@{}}
\toprule\noalign{}
Concern & Original design & End-product design & Rationale/effect \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Web boundary & React SPA plus Express API & Next.js pages and route handlers; legacy Express debt & Reduced CORS, session, and deployment complexity \\
Synchronization & Gmail watch/history and Graph webhooks/delta & Gmail history pulls and Outlook delta pulls & Preserved incremental sync without webhook infrastructure \\
AI provider & OpenAI and Anthropic & OpenAI Responses and Embeddings & Reduced adapter and evaluation scope \\
Search/RAG & FTS, vectors, attachment embeddings, citations & Provider search plus selected-thread, memory, same-contact, and message-vector context & Delivered scoped evidence; generalized grounding remains partial \\
Agent actions & Generic proposals with confirmation & Calendar, commitments, monitors, briefs, activity, cancel/retry & Concrete workflows shipped before the approval executor \\
Data model & Core mail, embeddings, tasks & Provider-neutral mail plus memory, feedback, calendars, commitments, briefs, activity, settings & Scope expanded into a communication workspace \\
Privacy & No third-party sharing & Minimized context sent to configured OpenAI service & Corrected incompatible wording and requires transparency \\
\end{longtable}

The complete public-ready design is maintained in
\texttt{docs/Relay\_System\_Design\_Document.tex} {[}21{]}.
...
\section{9.2 Prioritized Future Work}\label{92-prioritized-future-work}
...
\section{9.3 Closing Statement}\label{93-closing-statement}
