...
export function AiThreadAssistant({ messageId, subject, open, initialAction, onOpenChange, onInsertDraft }: AiThreadAssistantProps) {
  ...
  const run = async (action: AiAction, prompt?: string) => {
    ...
    setError(null)
    try {
      setResponse(await emailApi.runThreadAi({ messageId, action, prompt }))
