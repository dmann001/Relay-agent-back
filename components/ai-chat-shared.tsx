...
import { Check, ChevronDown, Clipboard, FileText, Globe2, Loader2, Mail, Plus, Save, Send, Square, Upload, Wrench, X } from "lucide-react"
...
import { emailApi, EmailApiError, type AiGeneratedImage, type AiModelOption, type AiModelSettings, type AiToolKey, WIRED_AI_TOOLS } from "@/lib/email-api"
...
export function useAiChat({ accountId, messageId, initialSessionId, onSessionId }: UseAiChatOptions) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      if (messageId && !emailDraftIntentPattern.test(question)) {
                        ...
                        <div className="mt-3 space-y-3 rounded-xl border border-border bg-card p-3 text-sm">
                          <div className="grid gap-2">
                            <div className="flex gap-2">
                              <span className="w-14 shrink-0 text-xs font-medium uppercase text-muted-foreground">To</span>
                              <span className="min-w-0 flex-1 text-foreground">
                                {message.draft.to.length ? message.draft.to.join(", ") : "Missing recipient"}
                              </span>
                            </div>
                            {message.draft.cc.length > 0 && (
                              <div className="flex gap-2">
                                <span className="w-14 shrink-0 text-xs font-medium uppercase text-muted-foreground">Cc</span>
                                <span className="min-w-0 flex-1 text-foreground">{message.draft.cc.join(", ")}</span>
                              </div>
                            )}
                            <div className="flex gap-2">
                              <span className="w-14 shrink-0 text-xs font-medium uppercase text-muted-foreground">Subject</span>
                              <span className="min-w-0 flex-1 text-foreground">
                                {message.draft.subject || "Missing subject"}
                              </span>
                            </div>
                          </div>
                          <div className="max-h-64 overflow-y-auto whitespace-pre-wrap rounded-lg border border-border bg-background p-3 leading-6 text-foreground">
                            {message.draft.body || "Missing email body"}
                          </div>
