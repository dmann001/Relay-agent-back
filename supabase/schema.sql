...
create table if not exists public.agent_memory (
  ...
  writing_profile jsonb not null default '{}'::jsonb,
  recent_context jsonb not null default '[]'::jsonb,
  profile_version int not null default 1,
  learning_enabled boolean not null default true,
  confirmed_learning_only boolean not null default true,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create index if not exists contacts_user_last_seen_idx on public.contacts(user_id, last_seen_at desc nulls last);
...
create index if not exists email_embedding_chunks_user_account_idx on public.email_embedding_chunks(user_id, account_id, created_at desc);
create index if not exists memory_items_user_status_idx on public.memory_items(user_id, status, updated_at desc);
create index if not exists memory_items_user_account_idx on public.memory_items(user_id, account_id, status, updated_at desc);
...
-- Personalization rows can only reference accounts owned by the user.
