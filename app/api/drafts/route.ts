...
export async function POST(request: NextRequest) {
  try {
    ...
    const snippet = draftBody.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 160);
    const row = {
      user_id: userId,
      ...
      if (account) {
        try {
          ...
          if (error?.code !== 404 && error?.response?.status !== 404) {
            console.error('[Drafts] Gmail delete failed (non-fatal):', error?.message);
          }
        }
      }
    }

    const { error } = await supabase.from('drafts').delete().eq('user_id', userId).eq('id', draftId);
    if (error) throw error;
